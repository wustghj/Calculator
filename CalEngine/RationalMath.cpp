...
namespace CalEngine
{
    ...
    Rational RationalMath::mod(const Rational &A, const Rational &B)
    {
        Rational Res;
        int32_t beMod = A.getInteger().toInt(); // 被mod数
        int Mod = B.getInteger().toInt(); // mod数
        if (A.getDecimal().size() == 0)
        {
            Res = Rational((int32_t)(beMod % Mod));
        }
        else
        {
            Res = Rational(beMod % Mod) + Rational(A.getDecimal());
        }
        return Res;
    }
